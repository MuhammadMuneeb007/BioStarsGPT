from unsloth import FastModel  # Use FastModel for Phi-4
import torch
import json
import os
import numpy as np
...
# ====== CONFIGURATION ======
train_samples = 144137  # Number of training samples to use
validation_samples = 10001  # Number of validation samples to use
...
load_in_4bit = True
# Use Phi-4 model
model_name = "unsloth/Phi-4"  # You can also use "unsloth/Phi-4-unsloth-bnb-4bit" for faster loading
num_epochs = 50        # Number of training epochs
output_dir = "outputs_phi" # Directory to save outputs
...
# Add LoRA adapters with Phi-4 appropriate settings
model = FastModel.get_peft_model(
    model,
    r=32,             # Rank parameter for LoRA
...
# ====== DATASET PREPARATION ======
# First, let's prepare the data in Phi-4 format
def preprocess_function(examples):
    # Extract prompts and completions
    prompts = examples["prompt"]
    completions = examples["completion"]
    
    # Format as Phi-4 conversations
    formatted_data = []
    for prompt, completion in zip(prompts, completions):
        formatted_data.append(f"<|im_start|>user<|im_sep|>{prompt}<|im_end|>\n<|im_start|>assistant<|im_sep|>{completion}<|im_end|>")
...
def evaluate_model(model, tokenizer, test_data, epoch):
    ...
    for i, item in enumerate(test_data):
        ...
        # Format message in Phi-4 format
        messages = [
        ...
        # Decode prediction - handle Phi-4's output format
        full_output = tokenizer.decode(outputs.sequences[0], skip_special_tokens=True)
        
        # Extract just the assistant's response (Phi-4 format)
        try:
            assistant_start = full_output.find("<|im_start|>assistant<|im_sep|>")
            if assistant_start != -1:
                prediction = full_output[assistant_start + len("<|im_start|>assistant<|im_sep|>"):]
                end_marker = prediction.find("<|im_end|>")
                if end_marker != -1:
                    prediction = prediction[:end_marker].strip()
            else:
                # If we can't find the markers, take everything after the user's prompt
...
# ====== TRAINING SETUP ======
# Using improved training arguments for the Phi-4 model
config = SFTConfig(
   ...
   learning_rate=2e-4,                  # Learning rate for Phi-4
   ...
   lr_scheduler_type="linear",          # Linear scheduler for Phi-4
...
early_stopping = EarlyStoppingCallback(patience=50000000000, min_delta=0.01)
gradient_monitor = GradientMonitoringCallback(model, log_freq=50)

# Setup trainer optimized for Phi-4
trainer = SFTTrainer(
   model=model,
   tokenizer=tokenizer,
...
# Use Unsloth's train_on_responses_only to only train on the assistant's responses
...
for epoch in range(1, num_epochs + 1):
   ...
   # Re-initialize trainer for next epoch with decayed learning rate
   if epoch < num_epochs:
       # Calculate new learning rate with linear decay
       progress = epoch / num_epochs
       new_lr = 2e-4 * (1 - progress)
       ...
       # Apply response-only training again
       trainer = train_on_responses_only(
           trainer,
           instruction_part="<|im_start|>user<|im_sep|>",
           response_part="<|im_start|>assistant<|im_sep|>",
...
# ====== SAVE FINAL MODEL ======
print("Saving final model to 'phi_4_finetuned'")
model.save_pretrained("phi_4_finetuned")
tokenizer.save_pretrained("phi_4_finetuned")
...
model.save_pretrained_merged("phi_4_deployment", tokenizer, save_method="merged_16bit")

# Save in 4bit for deployment
model.save_pretrained_merged("phi_4_deployment_4bit", tokenizer, save_method="merged_4bit")

# Save in GGUF format for llama.cpp
model.save_pretrained_gguf(
   "phi_4_gguf",
   tokenizer,
   quantization_type="q8_0",  # Options: q8_0, q4_k_m, q5_k_m, f16, bf16
)
"""
...
print(f"Final model saved to 'phi_4_finetuned'")
