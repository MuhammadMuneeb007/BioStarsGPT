...
load_in_4bit = True
# Use Qwen 2.5 model
model_name = "unsloth/Qwen2.5-7B"  # Can also use other sizes: 0.5B, 1.5B, 3B, 14B, etc.
num_epochs = 50        # Number of training epochs
output_dir = "outputs_qwen" # Directory to save outputs
...
print(f"Created validation split with {len(val_data)} samples")
...
# ====== DATASET PREPARATION ======
# Define the alpaca prompt format for Qwen models
...
# Function to format the data properly
def formatting_prompts_func(examples):
    instructions = examples["prompt"]
    inputs = ["" for _ in range(len(instructions))]  # Empty inputs
...
def evaluate_model(model, tokenizer, test_data, epoch):
    ...
    for i, item in enumerate(test_data):
        ...
        # Extract just the assistant's response by looking for "### Response:"
        try:
            response_start = full_output.find("### Response:")
            if response_start != -1:
                prediction = full_output[response_start + len("### Response:"):].strip()
            else:
                # If we can't find the marker, just use what was generated
...
# ====== TRAINING SETUP ======
# Using improved training arguments for the Qwen model
training_args = TrainingArguments(
   ...
   learning_rate=2e-4,               # Starting learning rate for Qwen
   ...
   lr_scheduler_type="linear",       # Linear scheduler for Qwen
...
early_stopping = EarlyStoppingCallback(patience=5000000000000000, min_delta=0.01)
gradient_monitor = GradientMonitoringCallback(model, log_freq=50)

# Setup trainer for Qwen
trainer = SFTTrainer(
   model=model,
   tokenizer=tokenizer,
...
# ====== SAVE FINAL MODEL ======
print("Saving final model to 'qwen_2_5_finetuned'")
model.save_pretrained("qwen_2_5_finetuned")
tokenizer.save_pretrained("qwen_2_5_finetuned")
...
model.save_pretrained_merged("qwen_2_5_deployment", tokenizer, save_method="merged_16bit")

# Save in 4bit for deployment
model.save_pretrained_merged("qwen_2_5_deployment_4bit", tokenizer, save_method="merged_4bit")
...
print("\n===== Testing final model =====")
test_prompt = "Continue the fibonnaci sequence: 1, 1, 2, 3, 5, 8"
...
print(f"Final model saved to 'qwen_2_5_finetuned'")
